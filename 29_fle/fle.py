...
def FLE_exec(files):
    ...
    for line in fle[".load"]:
        if line.startswith(f"{BYTES}:"):
            bs += bytes([int(b, 16) for b in line.removeprefix(f"{BYTES}:").split()])
    ...
    func_ptr = ctypes.c_void_p(
        ctypes.addressof(ctypes.c_char.from_buffer(mem)) + fle["symbols"]["_start"]
    )
        ...
        if "Relocation section" in line:
            ...
            if match := re.match(pattern, line):
                ...
                expr = " ".join(
                    [rs[0]]
                    + [
                        re.sub(r"([0-9a-fA-F]+)", lambda m: "0x" + m.group(1), r)
                        for r in rs[1:]
                    ]
                )
                expr += f" - {CURRENT}"
